...
bool WADArchive::Extract(const WADArchiveEntry& entry, wxOutputStream& oStr)
{
	...
	if (entry.GetSourceFileName().empty())
	{
		...
		if (entry.GetSourceArchive()) {
			archiveOffset = entry.GetSourceArchive()->m_dataOffset;
			//archiveOffset = entry.GetSourceArchive()->CalculateOffset();
		}
		else {
...
bool WADArchive::CreatePatch(const wxString& targetFileName)
{
	...
	if (patchArchive.Write())
	{
		...
		if(m_patchArchive)
			m_patchArchive->CalculateOffset();
		return true;
	}
	else
		return false;
...
void WADArchive::CalculateOffset(){
	...
	for (size_t fileIndex = 0; fileIndex < fileCount; fileIndex++)
	{
		...
		if (m_format != FmtHM1)
		{
			...
			if (m_entries[fileIndex].GetStatus() == WADArchiveEntry::EntryStatus::Entry_Original) {
				...
				m_entries[fileIndex].SetSize(fileSize);
			}
		}
		else
		{
